...
public class PlayerController : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        float threshold = 0.1f; // Вы можете подобрать значение порога экспериментальным путем.
    ...
    public void Attack()
    {
        ...
        if (isAtacket == true)
        {
            ...
            if (playerAnimator.isActiveAndEnabled)
            {
                ...
                {
                    // Прерываем анимацию и сбрасываем состояние
            ...
            // Если ни одного врага не было найдено, все равно переходим в состояние Idle
            StartCoroutine(AttackCooldown(0.8f));
        }
    }
